<?php
  ...
  if ($beginning<1426488300){
    $beginning=1426488300; //(Mar 16 2015)
  }
  ...
  while (isBeforeToday($curr_day)){
   	if ($res=$ll->db("SELECT * FROM ll2_events WHERE 
  					(timestamp>=$curr_day AND timestamp<=".getEndOfDay($curr_day)." AND cat1='work' AND cat2='misc' AND cat3='KR volunteer training' AND active=true)
      ...
      if ($curr_day_hours==0){
        ...
        $out.="<tr><td>".date("m/d - D",$curr_day)."</td><td class='value'>".number_format($curr_day_hours/HOUR,1)."</td></tr>";    
      }
	  ...
	  if ($bank>0) {
        ...
        if ($total_extra_hrs<0){
          $out.="<tr class='$wk_class'><td>Week total:<br/>$wk_dates</td><td class='value'>".number_format($curr_week_hours/HOUR,1)."</td></tr>";                
        }
        $curr_week_hours=0;          
      }        
    }
  ...
  $out="<table><tr class='headline'><td>Day</td><td>Hours worked</td></tr>".$out."</table>";
  
  $s->p("<div id='header'>King Road volunteer training, Johannes' hours from ".date("M j, Y",$calc_begin)." </div>");
  $s->p("<div id='start'><form>Want this calculation to start from a different day? Enter YYYY-MM-DD: <input type='text' name='beginning'><input type='submit' value='Go!'><br/><span style='font-size:70%'>Note: Calculation will start from the first Monday after the date entered."
    ."<br>Remember that more detailed information about each day's work may be accessed <a href='http://jowe.duckdns.org:499/~johannes/webservices/lifelog2/special/kr_training_project_details.php'>here</a>".
    "<br><span style='font-weight:bold'></span></span></span></form></div>");
