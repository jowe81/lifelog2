	...
	if ($res=$ll->db("SELECT * FROM ll2_events WHERE 
		...
		while ($r=mysqli_fetch_array($res)){
		    ...
		    if (($r["timestamp"]>$ts20181101) && ($r["timestamp"]<$ts20181217)){
		        ...
		        $threshould_over1=42;
		        $threshould_over2=45;		        
		    } else if ($r["timestamp"]>$ts20190901){
		        //40hrs
		        $threshould_under=38;
		        $threshould_over1=42;
		        $threshould_over2=45;		        
			...
			if (!isSameDay($lastts,$r["timestamp"])){
				if (!isSameWeek($lastts,$r["timestamp"])){
					if (isCurrentWeek($r["timestamp"])){
						...
						$week=0;
						if (!$is_last_entry){
						    $ae.="<div class='week_seperator'><a id='W".getBeginningOfWeek($r["timestamp"])."'></a>Week of ".date("l, F j",getBeginningOfWeek($r["timestamp"]))." to ".date("l, F j Y",getEndOfWeek($r["timestamp"]))."</div>";
						}
			...
			if (!$is_last_entry){
			    ...
			    */
			    ...
			    if ((!$r["cat5"]=="volunteer") && (($r["cat4"]=="STAT HOLIDAY") || ($r["cat4"]=="VACATION") || ($r["cat4"]=="RETREAT") || ($r["cat4"]=="SICK DAY") || ($r["cat4"]=="OFFICE CLOSURE"))){
			    ...
			    if ($r["cat5"]!="volunteer"){
			        $equivAvgWorkDay="(equiv. avg. work day: ".$dayLengthHrs."h)";
			    } else {
			        $equivAvgWorkDay="<span style='color:blue;'>(equiv. 0 work hours - volunteered)</span>";
			    }
			    
			    if ($r["cat4"]=="STAT HOLIDAY"){
			        $ae.="<div class='event' style='background-color:#FEE'><span style='color:#755'>STATUTORY HOLIDAY $equivAvgWorkDay</span></div>";
			    } else if ($r["cat4"]=="VACATION"){
			        $ae.="<div class='event' style='background-color:#FEE'><span style='color:#755'>VACATION DAY $equivAvgWorkDay</span></div>";
			        ...
			        $ae.="<div class='event' style='background-color:#EEF'><span style='color:#755'>RETREAT $equivAvgWorkDay</span></div>";
			    } else if ($r["cat4"]=="SICK DAY"){
			        $ae.="<div class='event' style='background-color:#FFE'><span style='color:#755'>SICK DAY $equivAvgWorkDay</span></div>";
			    } else if ($r["cat4"]=="OFFICE CLOSURE"){
			        $ae.="<div class='event' style='background-color:#FFE'><span style='color:#755'>OFFICE CLOSURE $equivAvgWorkDay</span></div>";
			    } else {			    
        			$ae.="<div class='event'>"
	...
	<body>
		...
		<div class="summary">
			<div class='summary_header'>
				<a class='nlink' href='tenth.php'>Go to Overview</a>
				Full Log
			</div>
    		<!--  
    		...
    		<div>Weekly avg. for period (hrs):
    			<div class='data'>
    				<?php
    				    $class="green";
    				    $avg=number_format(($totalBefore20181217/((xtime()-$ts20181101)/WEEK))/HOUR,1);
    				    if ($avg>42){
    				        $class="orange";
    				    }
    				    if ($avg>44){
