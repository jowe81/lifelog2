<?php
	...
	//Identify week by timestamp. 0=current week.
	$week=$_GET["w"];
	if ($week==0) {$week=time();} //Default to now - i.e. the newest entry in the past
	$start=getBeginningOfWeek($week);
	$end=$start+WEEK-1;
	$nextlink=$start+WEEK;
	$prevlink=$start-WEEK;
	//Shift to thursday:
	...
	if ($res=$ll->db("SELECT * FROM ll2_events WHERE 
					(timestamp>=$start AND timestamp<=$end AND cat1='friend support' AND cat2='piano lessons' AND person=35 AND active=true)
	...
	//Lahela
	if ($res=$ll->db("SELECT * FROM ll2_events WHERE 
					(timestamp>=$start AND timestamp<=$end AND cat1='friend support' AND cat2='piano lessons' AND person=5 AND active=true)
					ORDER BY timestamp DESC LIMIT 1
				;")){
		while ($r=mysql_fetch_array($res)){
					$allevents.="<tr>
	...
	//Salome
	if ($res=$ll->db("SELECT * FROM ll2_events WHERE 
					(timestamp>=$start AND timestamp<=$end AND cat1='friend support' AND cat2='piano lessons' AND person=34 AND active=true)
					ORDER BY timestamp DESC LIMIT 1
				;")){
		while ($r=mysql_fetch_array($res)){
					$allevents.="<tr>
	...
	$allevents.="</table>";
	$s->p("<p><span style='font-size:120%;font-style:italic;'>Piano lesson notes (".date("M d",$start+WEEK-DAY)." - ".date("M d, Y",$end+WEEK-DAY).")</span>&nbsp;-&nbsp;<span style='font-size:80%;'>Powered by LifeLog - A project of <a href='http://www.jowe.de/'>http://www.jowe.de</a></span>			
				<br/>
				
				<a href='?w=$prevlink'>previous week</a> | <a href='?w=$nextlink'>subsequent week</a> | <a href='?w=0'>current</a>
