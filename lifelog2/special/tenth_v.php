<?php
	...
	$week=0;
	$grandtotal=0;
	$week_v=0; //volunteering
	$grandtotal_v=0;//volunteering
	$thisweek=-1;
	if ($res=$ll->db("SELECT * FROM ll2_events WHERE 
					(timestamp>1520800000 AND timestamp<=".time().")
                        AND (cat1='work' AND cat2='Tenth Church' AND active=true)
                        OR (cat1='volunteering' AND cat2='Tenth Church' AND active=true)                    
			...
			if (!isSameDay($lastts,$r["timestamp"])){
				if (!isSameWeek($lastts,$r["timestamp"])){
					if (isCurrentWeek($r["timestamp"])){
					    ...
					    $volunteered="";
					    if ($week_v>0){
					        $volunteered="Volunteered: ".round(($week_v/HOUR),1)."h";
					    }
					    $ae.="<div class='week_total' style='float:left'>$graphic<span class='$c fixed_width_block'>$label ".round(($week/HOUR),1)."h</span></div>"
					        ."<div class='week_total_v bg_blue fixed_width_block' style=''><span>$volunteered</span></div>";
			    ...
			    */
			    if ($r["cat1"]=="work"){
    			    if ($r["cat4"]=="STAT HOLIDAY"){
    			        $ae.="<div class='event' style='background-color:#FEE'><span style='color:#755'>STATUTORY HOLIDAY (equiv. avg. work day: 6.4h)</span></div>";
    			        $statlength=round(60*60*6.4);
    			        $week+=$statlength;
    			        $grandtotal+=$statlength;
    			    } else if ($r["cat4"]=="RETREAT"){
    			        $ae.="<div class='event' style='background-color:#EEF'><span style='color:#755'>RETREAT (equiv. avg. work day: 6.4h)</span></div>";
    			        $statlength=round(60*60*6.4);
    			        $week+=$statlength;
    			        $grandtotal+=$statlength;			        
    			    } else if ($r["cat4"]=="SICK DAY"){
    			        $ae.="<div class='event' style='background-color:#FFE'><span style='color:#755'>SICK DAY (equiv. avg. work day: 6.4h)</span></div>";
    			        $statlength=round(60*60*6.4);
    			        $week+=$statlength;
    			        $grandtotal+=$statlength;
    			    } else {			    
			    ...
			    } else if ($r["cat1"]=="volunteering"){
			        $ae.="<div class='event' style='background-color:#EEF'>"
			            ."   <div class='event_time'>".date("H:i",$r["timestamp"])." - ".date("H:i",$r["timestamp"]+$r["duration"])." | ".getHumanReadableLengthOfTime($r["duration"])."</div><div class='event_cat'>".$calviews->getDeepestCategories($r,3,2)."</div>"
			                .""
			                    ."   <div class='event_notes'>".nl2br(htmlentities($r["notes"],ENT_QUOTES))."</div>"
			                        .$eventicon
