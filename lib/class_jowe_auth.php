...
class jowe_auth {
		...
		if ($res=$this->db("SELECT id from users where login='$login';"))
		{
			if (mysqli_num_rows($res)>0) {
				...
				$query="INSERT into users values (
							'',
							'".time()."',
							'".mysqli_real_escape_string($this->link,$login)."',
							'".mysqli_real_escape_string($this->link,$password)."',
							'".mysqli_real_escape_string($this->link,$lastname)."',
							'".mysqli_real_escape_string($this->link,$firstname)."',
							'".mysqli_real_escape_string($this->link,$email)."',
							'".mysqli_real_escape_string($this->link,$phone)."',
							'".mysqli_real_escape_string($this->link,$cell)."',
							'".mysqli_real_escape_string($this->link,$address)."',
							'".mysqli_real_escape_string($this->link,$zip)."',
							'".mysqli_real_escape_string($this->link,$country)."',
							'".mysqli_real_escape_string($this->link,$notes)."'
							);";
				echo $query;
	...
	public function create_session($user,$pass) {
		...
		if ($this->check_password($user,$pass)) {
			...
			//Write session info
			$insertstring="INSERT INTO sessions VALUES('','$new_sid','$user',1,'".$_SERVER['REMOTE_ADDR']."',".time().",".time().",true);";
			$d=$this->db($insertstring);
