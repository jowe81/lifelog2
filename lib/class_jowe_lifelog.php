...
class jowe_lifelog {
	...
	function connect() {
		if ($link=mysqli_connect($this->mysql_host,$this->mysql_user,$this->mysql_pass)) {
			//Link established
			if ($selected=mysqli_select_db($link,$this->mysql_database)) {
				//Everything's okay
				$this->link=$link;
				$this->connected=true;
			} else  {
	...
	public function build_update_query($e,$table) {
		...
		foreach ($e as $key=>$value) {
			if ($key!="id") {
				$q.=$key."='".mysqli_real_escape_string($this->link,$value)."',";
			}
		}
	...
	public function get_current_screenmessages($limit=0){
		...
		if ($res=$this->db("SELECT id,timestamp,billboard_text,cat4,cat5 FROM ll2_events WHERE cat1='notes' AND cat2='billboard' AND cat3='webinterface' AND active=true ORDER BY timestamp DESC $limit_condition;")){
			while ($q=mysqli_fetch_array($res,MYSQLI_ASSOC)){
	...
	public function set7SegClockBrightness($val){
	    $ip=$this->param_retrieve_value("7SGC_IP","7SEGCLOCK",true);
	    if ($ip!=""){	        
	       file_get_contents("http://$ip/write?brightness=$val");
	    }
	}
	
	public function set7SegClockLED($n,$newstatus){
	    $ip=$this->param_retrieve_value("7SGC_IP","7SEGCLOCK",true);
	    $newstatus ? $ns=1 : $ns=0;
	    $url="http://$ip/write?led=$n&state=$ns";
	    file_get_contents($url);	    
	    echo $url;
	}
	
	public function setMailboxLights($newstatus){
	    $ip=$this->param_retrieve_value("MAILBOX_IP","MAILBOX",true);
	    $newstatus ? $ns=1 : $ns=0;
	    if ($newstatus){
	        $url="http://$ip/write?lights=on";	        
	    } else {
	        $url="http://$ip/write?lights=off";	        
	    }	    
	    file_get_contents($url);
	    echo $url;	    
	}
	...
	public function toggle_statusflag($flagname){
		...
		if ($res=$this->record_statusflag($flagname,($newstatus))){
			...
			if ($flagname=="out_in"){
			    if (!$newstatus){
    				...
    				if (strlen($presetnr)>0){
    				    ...
    				    $this->pct_recall_preset($this->param_retrieve_value("PCT_PRESET_RETURN","POWERCONTROL",true));
    				}
			    }
				$this->set7SegClockLED(6,$newstatus);
			}
			...
			if ($flagname=="sleep_wake"){
			    if ($newstatus){
			        $this->pct_recall_preset("_SLEEP");
			        $this->set7SegClockBrightness($this->param_retrieve_value("7SGC_BRIGHTNESS_SLEEP","7SEGCLOCK",true));
			        $this->setMailboxLights(false);
			    } else {
			        $this->set7SegClockBrightness($this->param_retrieve_value("7SGC_BRIGHTNESS_WAKE","7SEGCLOCK",true));			        
			    }
			    $this->set7SegClockLED(2,$newstatus);
			}			
			if ($flagname=="busy_available"){
			    //$this->set7SegClockLED(2,$newstatus);			    
			}
		}
		return $res;
	...
	public function param_retrieve($pname,$pgroup="DEFAULT"){
		if ($res=$this->db("SELECT * FROM ll2_params WHERE pname='$pname' AND pgroup='$pgroup';")){
			if ($e=mysqli_fetch_array($res,MYSQLI_ASSOC)){
			    if (is_array($e)){
	...
	public function param_retrieve_value($pname,$pgroup="DEFAULT",$create_if_not_exists=false,$description_for_auto_create=" (auto created)"){
		if (isset($this->params[$pgroup][$pname])){
			return $this->params[$pgroup][$pname];
		} else {
		    if ($create_if_not_exists){
		        $this->param_store($pname,"",$pgroup,$description_for_auto_create);
		    }
			return false;
		}
	...
	function get_temp_from_local_sensors(){
		return file_get_contents("http://192.168.1.23/gettemp.php");
	}
	...
	function getSystemStatus($table=true){
		...
		if ($link=mysqli_connect($this->mysql_host,$this->mysql_user,$this->mysql_pass)) {
			//Connected to mysql
			if (mysqli_select_db($link,"jowe")){
				//Connected to jowe.de database
				//Total conns (hits>1 is to filter spambots)
				if ($res=mysqli_query($link,"SELECT * FROM jowe_sessions WHERE update_time>".(time()-130)." AND hits>1;")){
					$jowe_sessions_total=mysqli_num_rows($res);
				} else {
					$jowe_sessions_total="DB error";
				}
				//Remote conns
				if ($res=mysqli_query($link,"SELECT * FROM jowe_sessions WHERE update_time>".(time()-130)." AND NOT ((ip like '192.168.%') OR (ip like '127.%') OR (ip like '".$this->param_retrieve_value("CURRENT_IP","SYSTEM_STATUS")."%'))AND hits>1 ORDER BY update_time DESC;")){
					$jowe_sessions_remote=mysqli_num_rows($res);
		...
		//REMOTE VOLTAGE
		$t.="<tr><td style='text-align:left;'>Remote voltage</td><td style='text-align:right;'>".$this->param_retrieve_value("REMOTE_VOLTAGE","REMOTE")."</td></tr>";
	...
	function set_channel_status($n,$x){
		...
		//Channels 17-20 ESP living room window
		if (($n>=17) && ($n<=20)):
		    $local_id="relay_".($n-17);
		    shell_exec("curl \"http://192.168.1.23/switch?local_id=$local_id&state=$x\" > /dev/null 2>/dev/null &");
		endif;
		//Channels 21, 22, 23: ESP table
		if (($n>=21) && ($n<=23)):
    		$local_id="relay_0";
    		if ($n==22) { $local_id="relay_2"; }
    		if ($n==23) { $local_id="relay_1"; }
    		shell_exec("curl \"http://192.168.1.109/switch?local_id=$local_id&state=$x\" > /dev/null 2>/dev/null &");		
		endif;
		//Channels 24-27, ESP kitchen
		if (($n>=24) && ($n<=27)):
    		$local_id="relay_0";
    		if ($n==25) { $local_id="relay_1"; }
    		if ($n==26) { $local_id="relay_3"; } //Physical mismatch at relay board
				if ($n==27) { $local_id="relay_2"; }
    		shell_exec("curl \"http://192.168.1.21/switch?local_id=$local_id&state=$x\" > /dev/null 2>/dev/null &");
		endif;
	...
	function pct_recall_preset($n){
		...
		//Read preset name
		$ps_name=$this->param_retrieve_value("PCT_PRESET".$n."_NAME","POWERCONTROL");
		if ($ps_name=="movie"){
		  $this->set7SegClockBrightness($this->param_retrieve_value("7SGC_BRIGHTNESS_MOVIE","7SEGCLOCK",true));
		}
		
		$pa=array(); //Collect "p"-channels here
		for ($i=1;$i<(min(33,strlen($ps)+1));$i++){
			//Preset value for this channel
			$p=substr($ps,$i-1,1); 
			//Is this channel determined in the preset?
			if (($p=="0") || ($p=="1")){
				//Preset determines this channel (0 or 1, not * or x), so set it
				$this->set_channel_status($i,$p);
			}
			if ($p=="x"){
			    $this->pct_toggle_channel($i);
			}
			if ($p=="p"){
			    //add to "p" channel collection
			    array_push($pa,$i);
			}
		}
		if (count($pa)>0){
		    //Determine which way to switch p channels
		    $op=0; //Default to turning p channels off
		    for ($i=0;$i<count($pa);$i++){
		        if ($this->get_channel_status($pa[$i])==0){
		            $op=1; //Discovered a p channel that is off, so turn all of them on instead of off
		        }
		    }
		    for ($i=0;$i<count($pa);$i++){
		        $this->set_channel_status($pa[$i],$op);
		    }
		    //If lights were turned on, bring 7Seg brightness up
		    if ($op){
		        //$this->set7SegClockBrightness($this->param_retrieve_value("7SGC_BRIGHTNESS_WAKE","7SEGCLOCK",true));		        
		    }
		}
	...
	function getPCTstatus(){
		...
		for ($i=1;$i<57;$i++){
			//Position
			$pos="position:absolute;left:".((($i-1)%4)*25)."%;top:".(floor((($i-1)/4))*7.8)."%;";
			$content="";
			if ($i<33){
				//Channels
				//See if channel has name
				$name=$this->param_retrieve_value("PCT_CH".$i."_NAME","POWERCONTROL",true,"PowerControl Channel Name");
				//Get status
				...
				$name=$this->param_retrieve_value("PCT_PRESET".($i-32)."_NAME","POWERCONTROL",true);
				//Get status
				if ($this->pct_check_preset_activity($i-32)){			
					//Preset active
					$bg="#222266";
				} else {
					$bg="#444444";			
				...
				if ($name!=""){
					...
					<form id='f$i' action='".me()."&bb_width=".$_GET["bb_width"]."&bb_height=".$_GET["bb_height"]."&view=standard&processform=pct_recall_preset&preset=".($i-32)."' method='post'>
					<a style='color:#AAA;' href='javascript:{}' onclick=\"document.getElementById('f$i').submit(); return false;\">$name</a>
					</form>
					";
			...
			if ($name!=""){
    			$r.=" <div style='$pos width:25%;height:7%;'>
			    ...
			    $r.=" <div style='$pos width:25%;height:7%;'>
					<div style='position:relative;width:98%;height:100%;margin-left:auto;margin-right:auto;background:#222;'>
						&nbsp;
					</div>
				</div>";
	...
	function ring_doorbell(){
		$this->param_store("doorbell_last_ring",time(),"DOORBELL");
		$this->param_store("doorbell_answered",-1,"DOORBELL");
		$this->add_default_alert("doorbell.ogg");
	}
	
	function answer_door(){
		if (file_get_contents("http://192.168.1.200:8100/open")){
			$this->param_store("doorbell_answered",time(),"DOORBELL");
		}
	}
