...
class jowe_lifelog_calendar_views {
	...
	function display_event ($r,$enum,$mode="general",$content="regular",$fs=100,$delete_link_id="",$edit_link_id="",$nolinks=false){
		...
		$biked="";
		if ($r["value2"]==1) { $biked.="<br><img style='float:right;height:40px;width:40px;' src='img/bikeicon.png'/>";}
	...
	function dv_produce_daygrid_events($time){
		...
		if ($res=$this->ll->db($query)){
			if ($e=mysqli_fetch_array($res)){ //At least one record found.
		...
		switch($mode){
			...
			case "next_upcoming_event":
				$query="SELECT * from ll2_events WHERE
						...
						;";
				if ($res=$this->ll->db($query)){
					if ($q=mysqli_fetch_array($res,MYSQLI_ASSOC)){
			...
			//The next couple of upcoming events (not THE next one)
				...
				if ($res=$this->ll->db($query)){
					...
					$totalupcoming="";
					if ($res=$this->ll->db($query)){
						$totalupcoming=mysqli_num_rows($res);
					}
			...
			case "pct_status":
				...
				$r.="<div id='pct' style='position:relative;top:0px;left:0px;width:100%;height:100%;background:$bgcolor;'>
	...
	public function produce_billboard($width,$height,$time,$view="standard"){
		...
		if($this->ll->param_retrieve_value("MOVIEMODE","BILLBOARD")==0){
			...
			if ($view=="standard"){		
				...
				//Top right
				$screens[2]=$this->get_screen_array(($height*0.005),($width*0.7525),($width*0.245),($height*0.39),$this->produce_billboard_content("next_upcoming_events",$time,$fs*0.75),"black","none");
				//right middle
				$screens[9]=$this->get_screen_array(($height*0.40),($width*0.7525),($width*0.245),($height*0.345),$this->produce_billboard_content("pct_status",$time,$fs),"black","none");
				//Left middle
				...
				if (false && $this->ll->systemPanic()>.6){
		...
		return "<div id='outer' style='background:black; position:relative; margin-left:auto; margin-right:auto; width:".$width."px; height:".$height."px;'>$r</div>";
	}
